#include "multi_csrt.h"

MultiCSRT::MultiCSRT (ros::NodeHandle &n, bool view_img, int max_age, int min_hits, \
...
void MultiCSRT::callback (const sensor_msgs::ImageConstPtr &img_ptr, 
...
{ 
    ...
    else
    {
        ...
        {
            cv::Rect2d csrBox;
            bool active = it->csrDCF->update(img, csrBox);
            it->active = active;
            it->roi = csrBox;
            trks.push_back(csrBox);
        }
...
int main (int argc, char** argv)
{
    ...
    MultiCSRT multiCsrt(n, view_img, max_age, min_hits, psr_thresh, padding, filter_lr, template_size);
    cout << "csrDCF is ready" << endl;
    ...
    sync.registerCallback(boost::bind(&MultiCSRT::callback, &multiCsrt, _1, _2));
