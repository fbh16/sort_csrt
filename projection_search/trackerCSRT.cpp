...
bool TrackerCSRTImpl::updateImpl(const Mat& image_, Rect2d& boundingBox)
{
    ...
    bounding_box.x = object_center.x - current_scale_factor * original_target_size.width / 2.0f;  //left-top x
    bounding_box.y = object_center.y - current_scale_factor * original_target_size.height / 2.0f; //left-top y
    ...
    // {
    //     bounding_box.x = 0.8 * bounding_box.x + 0.2 * boundingBox.x; //left-top x
    //     bounding_box.y = 0.8 * bounding_box.y + 0.2 * boundingBox.y; //left-top y
    // }
    //用投影区域替换原始搜索区域
    if (boundingBox.x >= 0 && boundingBox.y >= 0)
    {
        std::cout << "-------------------------fusion func-------------------------." << std::endl;
        bounding_box.x = boundingBox.x - 0.5 * bounding_box.width;
        bounding_box.y = boundingBox.y - bounding_box.height;
        bounding_box.width = current_scale_factor * original_target_size.width;
        bounding_box.height = current_scale_factor * original_target_size.height;
    }
    //update tracker
