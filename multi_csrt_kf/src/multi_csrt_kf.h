...
public:
    ...
    MultiCsrtKf(ros::NodeHandle& n, bool view_img, int max_age, int min_hits, float psr_thresh, float padding, float filter_lr, int template_size);
    ...
    Track createTrack(int detIdx, cv::Mat img, ros::Time timeStamp)
    {
        ...
        KalmanTracker kf = KalmanTracker(dets[detIdx]);
